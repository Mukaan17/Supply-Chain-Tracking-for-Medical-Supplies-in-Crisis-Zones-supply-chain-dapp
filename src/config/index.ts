...
function getEnvVar(name: string, defaultValue: string | null = null): string | null {
...
function getNumberEnvVar(name: string, defaultValue: number | null = null): number | null {
  const value = process.env[name];
  if (value === undefined) return defaultValue;
...
const config = {
  ...
  network: {
    default: getEnvVar('REACT_APP_NETWORK', 'sepolia') || 'sepolia',
  ...
  performance: {
    cacheTTL: getNumberEnvVar('REACT_APP_CACHE_TTL', 300000) || 300000, // 5 minutes
    debounceDelay: 300,
    throttleDelay: 1000,
    maxCacheSize: 1000,
  ...
  transaction: {
    maxRetryAttempts: getNumberEnvVar('REACT_APP_MAX_RETRY_ATTEMPTS', 3) || 3,
    retryDelay: getNumberEnvVar('REACT_APP_RETRY_DELAY', 1000) || 1000,
    timeout: getNumberEnvVar('REACT_APP_TRANSACTION_TIMEOUT', 300000) || 300000, // 5 minutes
